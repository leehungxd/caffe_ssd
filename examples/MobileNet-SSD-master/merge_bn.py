...
if __name__ == '__main__':
    ...
    baseDir = '/caffe/caffe-ssd/models/MobileNet/anngic/SSD_300x300/'
    WithBnModel = baseDir + 'MobileNetSSD_deploy.prototxt'
    WithBnWeight = baseDir + 'Mobile_anngic_SSD_300x300_iter_5000.caffemodel'

    WithoutBnModel = baseDir + 'NoBnDeploy.prototxt'
    WithoutBnWeight = baseDir + 'NoBnWeight.caffemodel'

    pre_process(WithBnModel, WithoutBnModel)

    net = caffe.Net(WithBnModel, WithBnWeight, caffe.TEST)
    net2 = caffe.Net(WithoutBnModel, caffe.TEST)

    load_weights(net, net2)
    net2.save(WithoutBnWeight)
